# Final Verification Script - Memastikan rebrand Bandung Bersuara selesai dengan baik

$WorkspaceRoot = "c:\KULIAH\MAGANG\Magang di Perhutani\Bandung Bersuara"
...
$oldBrandingPatterns = @("Bandung Bersuara", "bandungbersuara", "Bandung Bersuara")
...
Write-Host "4. Checking for new branding 'Bandung Bersuara'..." -ForegroundColor Yellow
$newBrandingFound = Get-ChildItem -Path $WorkspaceRoot -Recurse -Include "*.html" -File |
    Select-String -Pattern "Bandung Bersuara|Bandung Bersuara|bandungbersuara" -ErrorAction SilentlyContinue |
    Where-Object { $_.Path -notlike "*\.bak.*" } | 
    Measure-Object

if ($newBrandingFound.Count -gt 0) {
    Write-Host "   ✅ Found 'Bandung Bersuara' branding in $($newBrandingFound.Count) places" -ForegroundColor Green
} else {
    Write-Host "   ⚠️  No 'Bandung Bersuara' branding found!" -ForegroundColor Yellow
    $issues += @{ Type = "Missing"; File = "All"; Reason = "No Bandung Bersuara branding found" }
}
...
foreach ($pkg in $pkgFiles) {
    ...
    if ($content -match '"name"\s*:\s*"bandungbersuara') {
        $pkgOK++
...
Write-Host ""
Write-Host "Rebrand Bandung Bersuara SELESAI" -ForegroundColor Green
